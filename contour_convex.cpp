...
void run_contour_convex() {
	...
	for (int i = 0; i < contours.size(); i++) {
		cv::putText(out, std::to_string(i), contours[i][0], 1, 1, cv::Scalar(100, 0, 255), 1);
		bool contourConvex = cv::isContourConvex(contours[i]);
		std::string convexStr = contourConvex ? "true" : "false";
		std::cout << "Contour " << i << " convex: " << convexStr << std::endl;
	}
